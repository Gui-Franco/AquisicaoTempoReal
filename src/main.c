...
void thread_transmissao(void *arg1, void *arg2, void *arg3)
{
    ...
    while(1) {
        if(k_msgq_get(&sensor_msgq, &msg, K_FOREVER) == 0) {
            LOG_INF("T:%u XR:%d YR:%d ZR:%d XF:%d YF:%d ZF:%d", 
                    msg.t, 
                    msg.x_raw, msg.y_raw, msg.z_raw,
                    msg.x_filt, msg.y_filt, msg.z_filt);
        }
    }
}
...
void thread_aquisicao(void *arg1, void *arg2, void *arg3)
{
    ...
    int32_t hist_x[6] = {0}, hist_y[6] = {0}, hist_z[6] = {0};
    uint8_t idx = 0;
    ...
    while (1) {
        ...
        hist_x[idx] = pacote.x_raw;
        hist_y[idx] = pacote.y_raw;
        hist_z[idx] = pacote.z_raw;
        idx = (idx + 1) % 6;
        ...
        pacote.x_filt = soma_x / 6;
        pacote.y_filt = soma_y / 6;
        pacote.z_filt = soma_z / 6;
...
K_THREAD_DEFINE(transmissao_tid, STACK_SIZE, thread_transmissao, NULL, NULL, NULL, 6, 0, 0);
K_THREAD_DEFINE(aquisicao_tid, STACK_SIZE, thread_aquisicao, NULL, NULL, NULL, 6, 0, 0);